...
def gaussian_mutation(chromosome: np.ndarray, prob_mutation: float,
    ...
    if scale:
        mutation[mutation_array] *= scale

    # Update
    chromosome[mutation_array] += mutation[mutation_array]
...
def uniform_mutation_with_respect_to_best_individual(chromosome: np.ndarray, best_chromosome: np.ndarray,
    ...
    """
    Randomly mutate each gene in an individual with probability, prob_mutation.
    If a gene is selected for mutation it will nudge towards the gene from the best individual.
    """
    mutation_array = np.random.random(chromosome.shape) < prob_mutation
