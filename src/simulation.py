...
class LabelsDisplayWidget(PygameLayout):
    def __init__(self):
        ...
        self.font = pg.font.Font(GUIConfig.font_family, GUIConfig.label_size)
...
class VisualizeFrame(PygameLayout):
    ...
    def update_neural(self):
        ...
        output = [1 if i == network_outputs[2].argmax() else 0 for i in range(4)]
        self.neural_vis.update_network([snake_feature, network_outputs[0], network_outputs[1], output])
...
def main():
    ...
    frame = VisualizeFrame(Simulation("train"))
    frame.run()
